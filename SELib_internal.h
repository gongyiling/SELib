...
// Read a number from file and cast it to UInt32
extern inline void __SELib_internal_WriteVariableLengthNumber(int numberLength, uint32_t *destination, SELIB_FS_HANDLE handle);

// Read a null-terminated string from file of any length
extern inline void __SELib_internal_WriteNullTerminatedString(uint8_t **src, SELIB_FS_HANDLE handle);

// Read a 3-dimensional vector from file (will cast to either float or double, depending on data flag and the setting in config)
extern inline void __SELib_internal_WriteVector3ToFile(uint8_t animationFlags, vec3_t *destination, SELIB_FS_HANDLE handle);

// Read a quaternion from file 
extern inline void __SELib_internal_WriteQuatToFile(uint8_t animationFlags, quat_t *destination, SELIB_FS_HANDLE handle);
